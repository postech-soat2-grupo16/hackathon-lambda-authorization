...
terraform {
  backend "s3" {
    ...
    key    = "hackathon-lambda-authorization/terraform.tfstate"
    region = "us-east-1"
...
resource "aws_security_group" "security_group_authorization_lambda_ecs" {
  name_prefix = "hackathon_security_group_authorization_lambda_ecs"
  description = "HACKATHON SG for Lambda Authoriation"
  vpc_id      = var.vpc_id
  ...
  tags = {
    infra   = "lambda"
    service = "gateway"
    Name    = "hackathon_security_group_authorization_lambda_ecs"
  }
}

## Infra lambda Authorization
resource "aws_lambda_function" "lambda_authorization" {
  function_name    = "hackathon-lambda-authorization"
  handler          = "lambda.main"
  runtime          = "python3.8"
  ...
  timeout          = 120
  description      = "Hackathon Lambda para autorizar"
  ...
  environment {
    ...
    tags = {
    infra   = "lambda"
    service = "hackathon-authorization"
  }
}
